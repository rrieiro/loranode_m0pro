...
/**
 * \fn      uint16_t f2sflt16(float f)
 * \brief   Encode a floating point number into a uint16_t.
 * \details The float to be transmitted must be a number in the range (-1.0, 1.0).
...
uint16_t f2sflt16(float f) {
    if (f <= -1.0)
        ...
        // we compute the fraction and then decide if we need to round.
...
/**
 * \fn      uint16_t f2sflt12(float f)
 * \brief   Encode a floating point number into a uint16_t using only 12 bits.
 * \details The float to be transmitted must be a number in the range (-1.0, 1.0).
 *          It is converted to 16-bit integer formatted as follows:
 *              bits 15-12: zero
 *              bit 11: sign
...
/**
 * \fn      uint16_t f2uflt16(float f)
 * \brief   Encode a floating point number into a uint16_t.
 * \details The float to be transmitted must be a number in the range [0, 1.0).
 *          It is converted to 16-bit integer formatted as follows:
...
/**
 * \fn      uint16_t f2uflt12(float f)
 * \brief   Encode positive floating point number into a uint16_t using only 12 bits.
 * \details The float to be transmitted must be a number in the range [0, 1.0).
 *          It is converted to 16-bit integer formatted as follows:
 *               bits 15-12: zero
